...
class Instance:
    ...
    def __init__(
        ...
        if limit is not None and fetch_limit is None and boost_limit is None:
            ...
            self.fetch_limit = (
                fetch_limit
                if fetch_limit is not None and fetch_limit > 0 and fetch_limit <= 20
                else 20
            )
            self.boost_limit = (
                boost_limit if boost_limit is not None and boost_limit > 0 else 4
            )
...
class Config:
    ...
    hashtag_diversity_enforced: bool = True
    max_boosts_per_hashtag_per_run: int = 1
    # Spam detection configuration
    spam_emoji_penalty: float = 0.5  # Points to reduce per emoji over the threshold
    spam_emoji_threshold: int = 2  # Number of emojis before penalty applies
    spam_link_penalty: float = 0.3  # Points to reduce when links are present
    # Quality threshold configuration
    min_score_threshold: float = (
        4  # Minimum raw score (before normalization) required for boosting
    )
    ...
    local_timeline_boost_limit: int = 4  # Max boosts from local timeline per run
    local_timeline_min_engagement: int = (
        1  # Minimum boosts, stars, or comments required
    )
    local_post_bonus: float = (
        1.5  # Multiplier for local instance posts (1.0 = no bonus, 2.0 = 2x bonus)
    )

    def __init__(self):
        # Helper method to get config values with environment variable override
        def get_config_value(
            env_var, config_dict, config_key, default_value, value_type=str
        ):
        ...
        with open(conf, "r") as configfile:
            ...
            self.interval = get_config_value(
                "HYPE_INTERVAL", config, "interval", self.interval, int
            )
            self.log_level = get_config_value(
                "HYPE_LOG_LEVEL", config, "log_level", self.log_level, str
            )
            self.debug_decisions = get_config_value(
                "HYPE_DEBUG_DECISIONS",
                config,
                "debug_decisions",
                self.debug_decisions,
                bool,
            )
            self.logfile_path = get_config_value(
                "HYPE_LOGFILE_PATH", config, "logfile_path", self.logfile_path, str
            )
            self.profile_prefix = get_config_value(
                "HYPE_PROFILE_PREFIX",
                config,
                "profile_prefix",
                self.profile_prefix,
                    ...
                    if "=" in pair:
                        ...
                        try:
                            ...
                            self.subscribed_instances.append(
                                Instance(name.strip(), limit=limit_int)
                            )
                        except ValueError:
                            logging.getLogger("Config").warning(
                                f"Invalid limit for instance {name}: {limit}"
                            )
            else:
                self.subscribed_instances = []
            ...
            self.filtered_instances = get_config_value(
                "HYPE_FILTERED_INSTANCES",
                config,
                "filtered_instances",
                self.filtered_instances,
                list,
            )
            if isinstance(self.filtered_instances, list) and config.get(
                "filtered_instances"
            ):
            ...
            self.daily_public_cap = get_config_value(
                "HYPE_DAILY_PUBLIC_CAP",
                config,
                "daily_public_cap",
                self.daily_public_cap,
                int,
            )
            self.per_hour_public_cap = get_config_value(
                "HYPE_PER_HOUR_PUBLIC_CAP",
                config,
                "per_hour_public_cap",
                self.per_hour_public_cap,
                int,
            )
            self.max_boosts_per_run = get_config_value(
                "HYPE_MAX_BOOSTS_PER_RUN",
                config,
                "max_boosts_per_run",
                self.max_boosts_per_run,
                int,
            )
            self.max_boosts_per_author_per_day = get_config_value(
                "HYPE_MAX_BOOSTS_PER_AUTHOR_PER_DAY",
                config,
                "max_boosts_per_author_per_day",
                self.max_boosts_per_author_per_day,
                int,
            )
            self.author_diversity_enforced = get_config_value(
                "HYPE_AUTHOR_DIVERSITY_ENFORCED",
                config,
            ...
            self.require_media = get_config_value(
                "HYPE_REQUIRE_MEDIA", config, "require_media", self.require_media, bool
            )
            self.skip_sensitive_without_cw = get_config_value(
                "HYPE_SKIP_SENSITIVE_WITHOUT_CW",
                config,
                "skip_sensitive_without_cw",
                self.skip_sensitive_without_cw,
                bool,
            )
            self.min_reblogs = get_config_value(
                "HYPE_MIN_REBLOGS", config, "min_reblogs", self.min_reblogs, int
            )
            self.min_favourites = get_config_value(
                "HYPE_MIN_FAVOURITES",
                config,
                "min_favourites",
                self.min_favourites,
            ...
            self.min_replies = get_config_value(
                "HYPE_MIN_REPLIES", config, "min_replies", self.min_replies, int
            )
            self.languages_allowlist = get_config_value(
                "HYPE_LANGUAGES_ALLOWLIST",
                config,
                "languages_allowlist",
                self.languages_allowlist,
                list,
            )
            self.use_mastodon_language_detection = get_config_value(
                "HYPE_USE_MASTODON_LANGUAGE_DETECTION",
                config,
                "use_mastodon_language_detection",
                self.use_mastodon_language_detection,
                bool,
            )
            self.state_path = get_config_value(
                "HYPE_STATE_PATH", config, "state_path", self.state_path, str
            )
            self.seen_cache_size = get_config_value(
                "HYPE_SEEN_CACHE_SIZE",
                config,
                "seen_cache_size",
            ...
            if hashtag_scores_env:
                ...
                for pair in hashtag_scores_env.split(","):
                    if "=" in pair:
                        ...
                        try:
                            self.hashtag_scores[tag.strip().lower()] = float(
                                score.strip()
                            )
                        except ValueError:
                            logging.getLogger("Config").warning(
                                f"Invalid score for hashtag {tag}: {score}"
                            )
            else:
                ...
                if config_hashtag_scores:
                    self.hashtag_scores = {
                        k.lower(): float(
                            v
                        )  # Changed to float to support negative values
                        for k, v in config_hashtag_scores.items()
                    }
                # If config doesn't specify hashtag_scores, keep the default from class attribute

            # Age decay configuration
            self.age_decay_enabled = get_config_value(
                "HYPE_AGE_DECAY_ENABLED",
                config,
                "age_decay_enabled",
                self.age_decay_enabled,
                bool,
            )
            self.age_decay_half_life_hours = get_config_value(
                "HYPE_AGE_DECAY_HALF_LIFE_HOURS",
                config,
                "age_decay_half_life_hours",
                self.age_decay_half_life_hours,
                float,
            )

            # Hashtag diversity configuration
            self.hashtag_diversity_enforced = get_config_value(
                "HYPE_HASHTAG_DIVERSITY_ENFORCED",
                config,
                "hashtag_diversity_enforced",
                self.hashtag_diversity_enforced,
                bool,
            )
            self.max_boosts_per_hashtag_per_run = get_config_value(
                "HYPE_MAX_BOOSTS_PER_HASHTAG_PER_RUN",
                config,
                "max_boosts_per_hashtag_per_run",
                self.max_boosts_per_hashtag_per_run,
                int,
            )

            # Spam detection configuration
            self.spam_emoji_penalty = get_config_value(
                "HYPE_SPAM_EMOJI_PENALTY",
                config,
                "spam_emoji_penalty",
                self.spam_emoji_penalty,
                float,
            )
            self.spam_emoji_threshold = get_config_value(
                "HYPE_SPAM_EMOJI_THRESHOLD",
                config,
                "spam_emoji_threshold",
                self.spam_emoji_threshold,
                int,
            )
            self.spam_link_penalty = get_config_value(
                "HYPE_SPAM_LINK_PENALTY",
                config,
                "spam_link_penalty",
                self.spam_link_penalty,
                float,
            )

            # Quality threshold configuration
            self.min_score_threshold = get_config_value(
                "HYPE_MIN_SCORE_THRESHOLD",
                config,
                "min_score_threshold",
            ...
            self.related_hashtags = (
                config.get("related_hashtags", self.related_hashtags) or {}
            )

            # Local timeline configuration
            self.local_timeline_enabled = get_config_value(
                "HYPE_LOCAL_TIMELINE_ENABLED",
                config,
                "local_timeline_enabled",
                self.local_timeline_enabled,
                bool,
            )
            self.local_timeline_fetch_limit = get_config_value(
                "HYPE_LOCAL_TIMELINE_FETCH_LIMIT",
                config,
                "local_timeline_fetch_limit",
                self.local_timeline_fetch_limit,
                int,
            )
            self.local_timeline_boost_limit = get_config_value(
                "HYPE_LOCAL_TIMELINE_BOOST_LIMIT",
                config,
                "local_timeline_boost_limit",
                self.local_timeline_boost_limit,
                int,
            )
            self.local_timeline_min_engagement = get_config_value(
                "HYPE_LOCAL_TIMELINE_MIN_ENGAGEMENT",
                config,
                "local_timeline_min_engagement",
                self.local_timeline_min_engagement,
                int,
            )
            self.local_post_bonus = get_config_value(
                "HYPE_LOCAL_POST_BONUS",
                config,
                "local_post_bonus",
                self.local_post_bonus,
                float,
            )
